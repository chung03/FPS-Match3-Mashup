...
public class FakeServerGameSend : MonoBehaviour
{
    ...
    public void SendDataIfReady(ref NativeList<NetworkConnection> connections, ref UdpNetworkDriver driver, Dictionary<int, ObjectWithDelta> IdToObjectsDictionary)
    {
	...
	private void HandleObjectDiff(Dictionary<int, ObjectWithDelta> IdToObjectsDictionary)
	{
		...
		SendDataToPlayerWhenReady(numObjectsToSend, CONSTANTS.SEND_ALL_PLAYERS);
			...
			if (playerQueue.Count > 0)
			{
				...
				using (var writer = new DataStreamWriter(playerQueue.Count, Allocator.Temp))
				{
					while (playerQueue.Count > 0)
					{
						...
						{
							Debug.Log("FakeServerGameSend::HandleIndividualPlayerSend connections[" + index + "] was not created");
							Assert.IsTrue(true);
						}
	...
	private void HandleAllPlayerSend(ref NativeList<NetworkConnection> connections, ref UdpNetworkDriver driver)
	{
		...
		for (int connectionIndex = 0; connectionIndex < connections.Length; ++connectionIndex)
		{
			...
			{
				Debug.Log("FakeServerGameSend::HandleAllPlayerSend connection[" + connectionIndex + "] was not created");
				Assert.IsTrue(true);
			}
			// Send eveyrthing in the queue
