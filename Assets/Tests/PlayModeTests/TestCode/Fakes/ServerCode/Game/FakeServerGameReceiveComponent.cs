...
public class FakeServerGameReceiveComponent : MonoBehaviour
{
	...
	private FakeServerGameDataComponent serverGameDataComponent;
	...
	{
		//Debug.Log("FakeServerGameReceiveComponent::HandleConnections Called");
		...
		for (int i = 0; i < connections.Length; i++)
		{
			...
			{
				Debug.Log("FakeServerGameReceiveComponent::HandleConnections Removing a connection");
		...
		{
			Debug.Log("FakeServerGameReceiveComponent::HandleConnections Too many connections, rejecting latest one");
			driver.Disconnect(c);
		}
	}
	...
	private void HandleReceiveData(ref NativeList<NetworkConnection> connections, ref UdpNetworkDriver driver)
	{
		for (int index = 0; index < connections.Length; ++index)
		{
			...
			{
				Debug.Log("FakeServerGameReceiveComponent::HandleReceiveData connections[" + index + "] was not created");
				Assert.IsTrue(true);
			}
					...
					NetworkEvent.Type.Empty)
			{
				...
				{
					Debug.Log("FakeServerGameReceiveComponent::HandleReceiveData Client disconnected from server");
					connections[index] = default;
				}
				else
				{
					Debug.Log("FakeServerGameReceiveComponent::HandleReceiveData Unhandled Network Event: " + cmd);
				}
			}

			//Debug.Log("FakeServerGameReceiveComponent::HandleReceiveData Finished processing connection[" + index + "]");
		}
	}
}
